...
let routes = [];
if (cmd == "serve") {
  routes = JSON.parse(fs.readFileSync("./src/routes.json", "utf-8"));
}
...
if (cmd == "reinstall") {
  ...
  if (fs.existsSync(`./${project}`)) {
    ...
    fs.writeFileSync(`${project}/dist/sw.js`, await fetch("https://ree.js.org/download/sw.js").then(r=>r.text()),"utf-8");
    let html = await fetch(
      "https://ree.js.org/download/index.html"
    ).then((res) => res.text());
  ...
  let css = findLine(cfg,"css");
  if(css="tw"){
    if(!fs.existsSync("./libs/tw.js.src")){
      fs.writeFileSync("./libs/tw.js.src",`url:https://ree.js.org/tw.js\nversion:0.0.1`);
    }
    if(!fs.existsSync("./libs/twcfg.js.src")){
      fs.writeFileSync("./libs/twcfg.js.src",`url:https://ree.js.org/twcfg.js\nversion:0.0.1`);
    }
    logger(`${css} is now installed!`,"info");
  }
} else if (cmd == "get") {
